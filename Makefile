...
CFLAGS += -I$(PROJECT_DIR) -DUSE_UPDATEENGINE=ON

ifdef RecoveryNoUi
LIBS = -lpthread
else
LIBS = -lz -lpng -ldrm -lpthread -lcurl -lcrypto
endif
...
$(UPDATE_ENGINE): $(UPDATE_ENGINE_OBJ)
	$(CC) -o $(UPDATE_ENGINE) $(UPDATE_ENGINE_OBJ) $(CFLAGS) $(LIBS)
...
install:
	...
	install -D -m 755 $(PROJECT_DIR)/rkRecovery $(DESTDIR)/usr/bin/
	install -D -m 755 $(PROJECT_DIR)/rkUpdateEngine $(DESTDIR)/usr/bin/
